...
from sklearn.naive_bayes import GaussianNB  # 拉普拉斯变换的贝叶斯模型
...
#data = pd.read_csv('creditcard_pca.csv')
# 数据预处理
# 特征和标签
...
def plot_roc_curve(fpr, tpr, roc_auc, label, color):
    ...
    #plt.savefig(f'课内_{label}_原_不调优')
...
for model_name, model in models.items():
    ...
    plt.xlabel('预测标签')
    plt.ylabel('真实标签')
    plt.savefig(f'课内_{model_name} 不采样_原_不调优_混淆矩阵')
    ...
    plt.xlabel('预测标签')
    plt.ylabel('真实标签')
    plt.savefig(f'课内_{model_name} 随机下采样_原_不调优_混淆矩阵')
    plt.close()  # 关闭图形，避免与后续图形冲突
    ...
    plt.xlabel('预测标签')
    plt.ylabel('真实标签')
    plt.savefig(f'课内_{model_name} 过采样_原_不调优_混淆矩阵')
    plt.close()  # 关闭图形，避免与后续图形冲突
